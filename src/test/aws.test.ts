...
describe('AWS S3 Operations', () => {
  ...
  describe('downloadFromS3', () => {
    ...
    it('should handle errors during file download', async () => {
      (S3Client.prototype.send as jest.Mock).mockReturnValue(Promise.reject(new Error('S3 error')));

      await expect(downloadFromS3('test-key', '/mock/download/path')).rejects.toThrow(
        'Error downloading file from S3: S3 error'
      );
  ...
  describe('uploadToS3', () => {
    it('should upload a file to S3 successfully', async () => {
      (S3Client.prototype.send as jest.Mock).mockReturnValue(Promise.resolve(undefined));
